  ...
  return withApiWriteObservability(
    ...
    async () => {
      ...
      if (!isSelfieObjectKey(selfieKey)) {
        return NextResponse.json(
          { message: "Selfie musÃ­ bÃ½t internÃ­ klÃ­Ä ÃºloÅ¾iÅ¡tÄ›." },
          { status: 400 },
        );
      }
      ...
      if (!isSelfieOwnedByUser(selfieKey, userId)) {
        return NextResponse.json(
          { message: "Selfie nepatÅ™Ã­ aktuÃ¡lnÃ­mu uÅ¾ivateli." },
          { status: 403 },
        );
      }
      ...
      try {
        ...
        console.error("OvÄ›Å™enÃ­ selfie v R2 selhalo:", error);
        return NextResponse.json(
          { message: "NepodaÅ™ilo se ovÄ›Å™it selfie." },
          { status: 500 },
        );
      }
